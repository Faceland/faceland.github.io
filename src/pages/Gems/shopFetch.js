...
// Tried in order; the first one that returns the shop's markup wins. corsproxy.io
// is gone — its free tier is localhost-only and 403s from face.land, so it could
// never have served a real visitor.
//
// Jina's reader needs X-Return-Format or it hands back Markdown instead of HTML,
// and X-No-Cache keeps the purchase ticker current. Both are allow-listed by its
// preflight, which the browser caches for 7h. No API key: the free tier is rate
// limited per IP, and every visitor fetches from their own.
const PROXIES = [
  {
    url: (url) => `https://r.jina.ai/${url}`,
...
const fetchText = async (url, headers, ms = 8000) => {
  const ctrl = new AbortController();
  const timer = setTimeout(() => ctrl.abort(), ms);
  try {
    const res = await fetch(url, { signal: ctrl.signal, credentials: 'omit', headers });
...
// Every blurb is boilerplate gemsData.js regenerates locally — except where a
// package carries its own "DISCLAIMER:" aside, which is prose nobody can derive
// from the numbers. Read it from the package's modal as TEXT (textContent, per
// the SECURITY note above — never markup), from the marker up to the standard
// NOTE that closes every blurb. '' when the package has no aside, which is the
// norm.
const parseDisclaimer = (doc, id) => {
  const body = doc.querySelector(`#package${id} .modal-body`)?.textContent || '';
  const aside = body.replace(/\s+/g, ' ').match(/DISCLAIMER:\s*(.*?)(?=\s*NOTE:|$)/i);
...
const parsePackages = (doc) => {
  ...
  doc.querySelectorAll('.item-wrapper').forEach((card) => {
    ...
    out.push({
      ...
      disclaimer: parseDisclaimer(doc, id),
    });
  });
  return out;
};
...
export const fetchLiveShopData = async () => {
  ...
  for (const proxy of PROXIES) {
    try {
      ...
      const text = await fetchText(proxy.url(CATEGORY_URL), proxy.headers);
